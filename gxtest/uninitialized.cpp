...
void TestTest()
{
  ...
  tref.colorchan0 = RasColorChan::Color1;
  ...
  for (int x = 0; x < 32; x++)
  {
    for (int y = 0; y < 32; y++)
    {
      ...
      if (x == y)
        DO_TEST(result.r == x, "Color was not set at x={}, y={} - got {:02x}, wanted {:02x}",
                x, y, result.r, x);
      else
        DO_TEST(result.r == 0xc0, "Color should not have been set at x={}, y={} - got {:02x}",
                x, y, result.r);
    }
  }
...
void TestUninitSimple()
{
  ...
  auto tref = CGXDefault<TwoTevStageOrders>(0);
  tref.colorchan0 = RasColorChan::Color0;
  CGX_LOAD_BP_REG(tref.hex);
  ...
  for (int x = 0; x < 32; x++)
  {
    ...
    DO_TEST(result.r == expected_color, "Wrong color at x={} - got {:02x}, expected {:02x}",
...
void TestUninitIncrement()
{
  ...
  for (int x = 0; x < 32; x++)
  {
    ...
    DO_TEST(result.r == 16*7, "Wrong color at x={}, y=1 - got {:02x}, expected {:02x}",
            x, result.r, 16*7);
    result = GXTest::ReadTestBuffer(x, 2, 32);
    DO_TEST(result.r == (16^8)*7, "Wrong color at x={}, y=2 - got {:02x}, expected {:02x}",
            x, result.r, (16^8)*7);
  }
...
void TestUninitSeparate()
{
  ...
  for (int x = 0; x < 32; x++)
  {
    ...
    DO_TEST(result_0.r == expected_0, "Wrong color 0 at x={}, y=1 - got {:02x}, expected {:02x}",
            x, result_0.r, expected_0);
    DO_TEST(result_1.r == expected_1, "Wrong color 1 at x={}, y=2 - got {:02x}, expected {:02x}",

...
#define SCISSOR_FMT "x0 %4d y0 %4d x1 %4d y1 %4d xOff %4d yOff %4d"
#define SCISSOR_ARGS(s) scissor.x0, scissor.y0, scissor.x1, scissor.y1, scissor.x_off<<1, scissor.y_off<<1
...
bool CheckViewportRect(const Scissor& scissor, Rect<u32> rect)
{
  ...
  if (!(tl || tr || bl || br))
  {
    ...
    DO_TEST(false, SCISSOR_FMT ": Expected rect at %d %d %d %d",
            SCISSOR_ARGS(scissor), x0, y0, x1, y1);
    return false;
  }
  else if (tl && tr && bl && br)
  {
    ...
    {
      DO_TEST(false, SCISSOR_FMT ": Found rect at %d %d %d %d but actual rect was bigger",
              SCISSOR_ARGS(scissor), x0, y0, x1, y1);
      return false;
    }
    else
    {
      DO_TEST(true, SCISSOR_FMT ": Found rect at %d %d %d %d",
              SCISSOR_ARGS(scissor), x0, y0, x1, y1);
      return true;
    }
  }
  else
  {
    ...
    DO_TEST(false, SCISSOR_FMT ": Expected rect at %d %d %d %d but actual rect was smaller",
            SCISSOR_ARGS(scissor), x0, y0, x1, y1);
    return false;
  }
}
...
void DoTest(bool add_gx_offset, int x0, int y0, int x1, int y1, int x_off, int y_off)
{
  ...
  if (!expected.empty())
  {
    ...
    DO_TEST(num_ok == expected.size(), SCISSOR_FMT ": %d of %d rects matched",
            SCISSOR_ARGS(scissor), num_ok, (u32)expected.size());
  }
  else
  {
    DO_TEST(IsTestBufferCleared(), SCISSOR_FMT ": There should be no rects",
            SCISSOR_ARGS(scissor));
  }
}

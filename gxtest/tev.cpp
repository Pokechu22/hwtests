...
void TevCombinerTest()
{
  ...
  for (int i = 0; i < 2; ++i)
    for (tevreg.ra.red = -1024; tevreg.ra.red != 1023; tevreg.ra.red = tevreg.ra.red + 1)
    {
      ...
      if (i == 0)
      {
        ...
        DO_TEST(result == tevreg.ra.red, "Got %d, expected %d", result, (s32)tevreg.ra.red);
      }
      else
      {
        ...
        DO_TEST(result == expected, "Run %d: Got %d, expected %d", (int)tevreg.ra.red, result,
                expected);
      }
    }
  ...
  for (int i = 0x000000; i < 0x000F000; ++i)
  {
    ...
    DO_TEST(result == expected, "Mismatch on a=%d, b=%d, c=%d, d=%d, shift=%d, bias=%d, op=%d, "
            ...
            a, b, c, d, (u32)cc.scale.Value(), (u32)cc.bias.Value(), (u32)cc.op.Value(), (u32)cc.clamp.Value(), expected, result);
...
void KonstTest()
{
  ...
  // Test the behavior of TevColorArg::Half.
  {
    ...
    DO_TEST(result.r == 128, "TevColorArg::Half test failed; actual %d", result.r);
